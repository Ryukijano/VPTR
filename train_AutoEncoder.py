...
if __name__ == '__main__':
    ckpt_save_dir = Path('D:\\video_dataset\\VPTR\\MovingMNIST\\VPTR_ckpts\\MNIST_ResNetAE_MSEGDLgan_ckpt')
    tensorboard_save_dir = Path('D:\\video_dataset\\VPTR\\MovingMNIST\\VPTR_ckpts\\MNIST_ResNetAE_MSEGDLgan_tensorboard')
    ...
    dataset_dir = 'D:\\video_dataset\\VPTR\\moving-mnist-example'
...
#     tensorboard_save_dir = Path('tensorboard_logs')
#     resume_ckpt = None
#     start_epoch = 0
#     summary_writer = SummaryWriter(tensorboard_save_dir.absolute().as_posix())
#     num_past_frames = 10
...
#     gdl_loss = GDL(alpha=1)
#     if resume_ckpt is not None:
#         loss_dict, start_epoch = resume_training({'VPTR_Enc': VPTR_Enc, 'VPTR_Dec': VPTR_Dec, 'VPTR_Disc': VPTR_Disc}, {'optimizer_G': optimizer_G, 'optimizer_D': optimizer_D}, loss_name_list, resume_ckpt)
...
#         # Defining a sample from the training loader for visualization
#         train_sample = next(iter(train_loader), None)
#         if train_sample is not None:
#             show_samples(VPTR_Enc, VPTR_Dec, train_sample, ckpt_save_dir.joinpath(f'train_gifs_epoch{epoch}'), renorm_transform)
...
#         # Defining a sample from the validation loader for visualization
#         val_sample = next(iter(val_loader), None)
#         if val_sample is not None:
#             show_samples(VPTR_Enc, VPTR_Dec, val_sample, ckpt_save_dir.joinpath(f'val_gifs_epoch{epoch}'), renorm_transform)
...
#     # Test
#     test_sample = next(iter(test_loader), None)
#     if test_sample is not None:
#         show_samples(VPTR_Enc, VPTR_Dec, test_sample, ckpt_save_dir.joinpath(f'test_gifs_epoch{epoch}'), renorm_transform)
